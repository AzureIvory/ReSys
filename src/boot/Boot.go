...
var (
	...
	runBCD              = func(args ...string) (string, error) {
		bin := utils.GetSystemExe("bcdedit.exe")
		out, err := tools.RunCmd(bin, nil, nil, "", args...)
		if err != nil && (errors.Is(err, os.ErrNotExist) || errors.Is(err, exec.ErrNotFound)) {
			if exe, e := os.Executable(); e == nil {
				bin = filepath.Join(filepath.Dir(exe), "tools", "bcdedit.exe")
				out, err = tools.RunCmd(bin, nil, nil, "", args...)
			}
		}
		if err != nil {
			return out, fmt.Errorf("bcdedit failed (%s): %w", bin, err)
		}
		return out, nil
	}
	getFw = GetFwType
...
// SetFwOnce 按 PE 文件所在分区匹配已存在的 UEFI 固件项，
// 并设置为下次启动一次性进入该固件项。
func SetFwOnce(pePath string) (string, error) {
	pePath = strings.TrimSpace(pePath)
	if pePath == "" {
		return "", fmt.Errorf("pe path is empty")
	}
	if abs, err := filepath.Abs(pePath); err == nil {
		pePath = abs
	}
	...
	out, err := runBCD("/enum", "firmware", "/v")
	if err != nil {
		return "", err
	}
	list, err := parseFwList(out)
	if err != nil {
		return "", err
	}
	it, err := pickFwEntry(list, pePath)
	if err != nil {
		return "", err
	}

	if _, err := runBCD("/set", "{fwbootmgr}", "bootsequence", it.ID); err != nil {
		return "", err
	}
	log.LogWrite(0, "[SetFwOnce] pe=%s id=%s dev=%s path=%s", pePath, it.ID, it.Device, it.Path)
	return it.ID, nil
}

func parseFwList(out string) ([]fwItem, error) {
	const (
		zhID   = "\u6807\u8bc6\u7b26"
		zhDev  = "\u8bbe\u5907"
		zhPath = "\u8def\u5f84"
	...
	isKeyLine := func(s string) bool {
		low := strings.ToLower(strings.TrimSpace(s))
		return strings.HasPrefix(low, "identifier") ||
			strings.HasPrefix(low, "device") ||
			strings.HasPrefix(low, "path") ||
			strings.HasPrefix(low, "description") ||
			strings.HasPrefix(s, zhID) ||
			strings.HasPrefix(s, zhDev) ||
			strings.HasPrefix(s, zhPath) ||
			strings.HasPrefix(s, zhDesc)
	}
	flush := func(dst []fwItem, cur fwItem) []fwItem {
		if cur.ID == "" {
			return dst
		}
	...
	for _, raw := range strings.Split(out, "\n") {
		...
		key := fs[0]
		val := strings.Join(fs[1:], " ")
		low := strings.ToLower(key)

		switch {
		case low == "identifier" || key == zhID:
			cur.ID = fs[len(fs)-1]
		case low == "device" || key == zhDev:
			cur.Device = val
		case low == "path" || key == zhPath:
			cur.Path = val
		case low == "description" || key == zhDesc:
			cur.Desc = val
		}
	}
	list = flush(list, cur)
	if len(list) == 0 {
		return nil, fmt.Errorf("no firmware entries found")
	}
	return list, nil
}

func pickFwEntry(list []fwItem, pePath string) (fwItem, error) {
	vol := strings.ToUpper(filepath.VolumeName(filepath.Clean(pePath)))
	if vol == "" {
		return fwItem{}, fmt.Errorf("invalid pe path: %s", pePath)
	}

	wantPart := strings.ToUpper("partition=" + vol)
	wantFile := strings.ToLower(filepath.Base(pePath))
	best := fwItem{}
	bestScore := 0
	hitCnt := 0
	...
	for _, it := range list {
		...
		score := 0
		dev := strings.ToUpper(strings.TrimSpace(it.Device))
		pth := strings.ToLower(strings.TrimSpace(it.Path))
		if dev == wantPart {
			score += 100
		} else if strings.Contains(dev, wantPart) {
			score += 80
		}
		if wantFile != "" && strings.Contains(pth, wantFile) {
			score += 20
		}
		if score == 0 {
			continue
	...
	if bestScore == 0 {
		return fwItem{}, fmt.Errorf("no matched firmware entry for %s", pePath)
	}
	if hitCnt > 1 {
		return fwItem{}, fmt.Errorf("multiple firmware entries matched %s", pePath)
	}
	return best, nil
}

func skipFwItem(it fwItem) bool {
	id := strings.ToLower(strings.TrimSpace(it.ID))
	desc := strings.ToLower(strings.TrimSpace(it.Desc))
	if id == "{fwbootmgr}" || id == "{bootmgr}" {
		return true
	}
	return strings.Contains(desc, "windows boot manager")
}
