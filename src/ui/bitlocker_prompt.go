...
//
// 重要约束：
// UiBitLockerUnlock 是“阻塞式”等待用户输入的 API，必须在非 UI 线程调用。
...
// UiBitLockerUnlock 在 UI 中显示 BitLocker 解锁提示，并阻塞等待用户输入。
//
// 返回值约定：
...
func UiBitLockerUnlock(title, prompt string) (credential string, useRecoveryKey bool, canceled bool, err error) {
	if ui.app == nil {
	...
	if err := ui.app.Post(func() {
		showBitLocker(title, prompt, resultCh)
	}); err != nil {
...
// showBitLocker 在 UI 线程内生效：切换页面、模糊背景，并把 prompt 字段写入 Store。
func showBitLocker(title, prompt string, resultCh chan bitLockerPromptResult) {
...
// bitLockerInput 由 Edit 输入框的 action 回调触发。
// 为了避免“用户输入后仍显示旧错误提示”，这里会同时清空 `prompt.error`。
func bitLockerInput(value string) {
	if ui.store == nil {
		return
	}
	ui.store.Patch(map[string]any{
...
// submitBitLocker 由“确认”按钮触发。
// 如果输入为空，提示错误并保持 prompt 可见；否则把结果回传并关闭 prompt。
func submitBitLocker(useRecoveryKey bool) {
	if !ui.bitLockerPromptVisible {
		return
	}
	...
	resolveBitLocker(bitLockerPromptResult{
		Credential:     credential,
		UseRecoveryKey: useRecoveryKey,
	})
}

// cancelBitLocker 由“取消”按钮触发。
func cancelBitLocker() {
	if !ui.bitLockerPromptVisible {
		return
	}
	resolveBitLocker(bitLockerPromptResult{Canceled: true})
}

// bitlockerClean 用于窗口销毁时的兜底清理（见 Ui.go 的 onDestroy）。
...
// resolveBitLocker 统一收口：隐藏 prompt、恢复背景效果，并回传结果。
//
// 这里会 close 掉 resultCh，调用方的 `<-resultCh` 会解除阻塞。
func resolveBitLocker(result bitLockerPromptResult) {
