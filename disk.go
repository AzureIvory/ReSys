...
func SplitVolume1(vol string, sizeMB int, fs, label string) (string, error) {
	...
	// 收缩分区，在右侧产生 sizeMB 未分配空间：
	// PartAssist: /resize:F /reduce-right:1000
	...
	// 如果尾部不是 MB 对齐，offset 上取整会吃掉 <1MB，size 需要相应减 1 才更稳
	createSize := sizeMB
