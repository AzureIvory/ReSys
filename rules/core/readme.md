...
- `Source`：来源标识。建议填写稳定且可读的名字，未填写时解析器默认使用规则文件名。
- `Rank`：来源优先级。使用整数，值越大越靠前；解析器在聚合镜像、PE、微软直链时都会先按 `Rank` 排序，再做去重。
- `Enabled`：是否启用当前规则。未填写时默认 `true`；当值为 `false` 时，规则文件仍可被单独解析测试，但不会参与目录扫描聚合。
- `direct/` 目录只表示“这是直链规则”，不再自动拥有更高优先级。是否优先完全由 `Rank` 决定。
- 当多个来源产生相同结果时，会保留更高 `Rank` 的那一条；同 `Rank` 时再按来源名、文件名等稳定字段排序。

`section_kv_group_v1`、`items`、`rules` 三种模式都支持这三个根字段。推荐所有来源规则都显式填写：
